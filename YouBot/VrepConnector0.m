classdef VrepConnector0
    ...
    methods
        function obj = VrepConnector0(port, step_time_vrep)
            ...
            % Retrieve handles for each joint (assuming your robot has 5 joints)
            ...
            % Initialize joint positions streaming
            for i = 1:5
        ...
        function ApplyControl(obj, u, delta_t)
            % Apply control to each joint with target velocities from vector u
            for i = 1:5
        ...
        function q = GetState(obj)
            % Retrieve the current position of each joint
            q = zeros(5, 1);
            for i = 1:5
